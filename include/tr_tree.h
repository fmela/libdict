...
tr_tree *tr_tree_new(dict_cmp_func key_cmp, dict_del_func key_del,
					 dict_del_func dat_del);
dict	*tr_dict_new(dict_cmp_func key_cmp, dict_del_func key_del,
					 dict_del_func dat_del);
void	 tr_tree_destroy(tr_tree *tree, int del);

int tr_tree_insert(tr_tree *tree, void *key, void *dat, int overwrite);
int tr_tree_probe(tr_tree *tree, void *key, void **dat);
void *tr_tree_search(tr_tree *tree, const void *key);
const void *tr_tree_csearch(const tr_tree *tree, const void *key);
int tr_tree_remove(tr_tree *tree, const void *key, int del);
void tr_tree_empty(tr_tree *tree, int del);
void tr_tree_walk(tr_tree *tree, dict_vis_func visit);
...
int tr_itor_set_data(tr_itor *itor, void *dat, int del);
